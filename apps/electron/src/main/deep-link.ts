 ...
 * Deep Link Handler
 *
 * Parses zenskill:// URLs and routes to appropriate actions.
 *
 * URL Formats (workspace is optional - uses active window if omitted):
 *
 * Compound format (hierarchical navigation):
 *   zenskill://allSessions[/session/{sessionId}]            - Session list (all sessions)
 *   zenskill://flagged[/session/{sessionId}]             - Session list (flagged filter)
 *   zenskill://state/{stateId}[/session/{sessionId}]     - Session list (state filter)
 *   zenskill://sources[/source/{sourceSlug}]          - Sources list
 *   zenskill://settings[/{subpage}]                   - Settings (general, shortcuts, preferences)
 *
 * Action format:
 *   zenskill://action/{actionName}[/{id}][?params]
 *   zenskill://workspace/{workspaceId}/action/{actionName}[?params]
 *
 * Actions:
 ...
 *
 * Examples:
 *   zenskill://allSessions                               (all sessions view)
 *   zenskill://allSessions/session/abc123                (specific session)
 *   zenskill://settings/shortcuts                     (shortcuts page)
 *   zenskill://sources/source/github                  (github source info)
 *   zenskill://action/new-chat                        (uses active window)
 *   zenskill://action/resume-sdk-session/{sdkId}      (resume Claude Code session)
 *   zenskill://workspace/ws123/allSessions/session/abc123   (targets specific workspace)
 */

import type { BrowserWindow } from 'electron'
import { SCHEME } from '@craft-agent/shared/brand'
...
export function parseDeepLink(url: string): DeepLinkTarget | null {
  try {
    ...
    // e.g., zenskill://workspace/ws123 → hostname='workspace', pathname='/ws123'
    // e.g., zenskill://allSessions/chat/abc → hostname='allSessions', pathname='/chat/abc'
    const host = parsed.hostname
    const pathParts = parsed.pathname.split('/').filter(Boolean)
    const windowMode = parseWindowMode(parsed)
    const rightSidebar = parseRightSidebar(parsed)

    // zenskill://auth-callback?... (OAuth callbacks - return null to let existing handler process)
    ...
    // zenskill://allSessions/..., zenskill://settings/..., etc. (compound routes)
    ...
    // zenskill://workspace/{workspaceId}/... (with workspace targeting)
    if (host === 'workspace') {
    ...
    // zenskill://action/... (no workspace - uses active window)
    if (host === 'action') {
