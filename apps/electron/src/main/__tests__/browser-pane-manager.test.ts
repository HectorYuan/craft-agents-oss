...
describe('BrowserPaneManager', () => {
  ...
  beforeEach(() => {
    ...
    manager.setCdpFactory(() => new MockBrowserCDP() as any)
  })
  ...
  it('denies app deep-link popups and forwards to deep-link handler', async () => {
    ...
    await Bun.sleep(0)
    expect(mockShellOpenExternal).toHaveBeenCalledWith('zenskill://settings')
  })
  ...
  it('retries toolbar load and recovers', async () => {
    ...
    expect(toolbarWc.loadURL).not.toHaveBeenCalledWith(expect.stringContaining('browser-toolbar.html'))
  })
