...
export async function handleConfigValidate(
  ...
  // Config root: explicit env override (new ZenSkill alias first, legacy
  // CRAFT_CONFIG_DIR kept for compatibility), else the ZenSkill default.
  // (No import from @craft-agent/shared here — this package must stay below it.)
  const appConfigRoot =
    process.env.ZENSKILL_CONFIG_DIR || process.env.CRAFT_CONFIG_DIR || join(homedir(), '.zenskill');
