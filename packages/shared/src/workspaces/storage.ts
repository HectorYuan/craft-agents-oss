 ...
 * Default location: ~/.zenskill/workspaces/
 */
...
/**
 * Get the default workspaces directory (~/.zenskill/workspaces/)
 */
 ...
 * @param baseDir - Parent directory where workspace folders live (e.g., ~/.zenskill/workspaces/)
 ...
 * Returns paths to valid workspaces found in ~/.zenskill/workspaces/
 */
...
export function ensurePluginManifest(rootPath: string, workspaceName: string): void {
  ...
  const manifest = {
    name: `zenskill-workspace-${workspaceName.toLowerCase().replace(/[^a-z0-9]+/g, '-')}`,
    version: '1.0.0',
  };
