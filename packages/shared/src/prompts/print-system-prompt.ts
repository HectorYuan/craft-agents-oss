#!/usr/bin/env bun
/**
 * Debug script to print the full ZenSkill system prompt with annotations.
...
${colors.bgMagenta}${colors.bold}                    ZENSKILL SYSTEM PROMPT BREAKDOWN                         ${colors.reset}
${colors.bgMagenta}${colors.bold}                                                                                ${colors.reset}
`);
...
const systemPrompt = getSystemPrompt(
  ...
  '/Users/example/.zenskill/workspaces/abc123' // Example workspace path
);
...
const systemPromptWithDebug = getSystemPrompt(
  undefined,
  { enabled: true, logFilePath: '~/Library/Logs/zenskill/main.log' },
  '/Users/example/.zenskill/workspaces/abc123'
);
...
const sessionState = formatSessionState('260121-example-session', {
  plansFolderPath:
    '/Users/example/.zenskill/workspaces/abc123/sessions/260121-example-session/plans',
});
...
printAnnotation('Generated by the agent runtime formatSourceState() - requires agent instance');
...
${colors.bold}SDK Configuration:${colors.reset}
  ...
  systemPrompt.append: getSystemPrompt() ${colors.dim}// ZenSkill additions (static, cacheable)${colors.reset}
...
${colors.bold}Static System Prompt Components:${colors.reset}
  ...
  2. ZenSkill Environment Marker        ${colors.dim}// Version, platform, arch${colors.reset}
