 ...
 *
 * Catalog entries live in models-zenskill.ts (zenskill-only distribution).
 * Pi has no hardcoded entries — models are discovered dynamically:
 *   - Pi: getModels(provider) from @earendil-works/pi-ai SDK
 * See ModelRefreshService in apps/electron/src/main/model-fetchers/
 */
export const MODEL_REGISTRY: ModelDefinition[] = ZENSKILL_MODEL_REGISTRY;
