...
describe('buildOAuthDeeplinkUrl', () => {
  it('returns deeplink URL when sessionId and deeplinkScheme are provided', () => {
    const ctx: OAuthSessionContext = {
      ...
      deeplinkScheme: 'zenskill',
    };
    expect(buildOAuthDeeplinkUrl(ctx)).toBe('zenskill://allSessions/session/260209-swift-river');
  });
  ...
  it('uses custom deeplink scheme for multi-instance', () => {
    const ctx: OAuthSessionContext = {
      ...
      deeplinkScheme: 'zenskill1',
    };
    expect(buildOAuthDeeplinkUrl(ctx)).toBe('zenskill1://allSessions/session/test-session');
  });
  ...
  it('handles session ID with special characters (hyphens and numbers)', () => {
    ...
    expect(buildOAuthDeeplinkUrl(ctx)).toBe('zenskill://allSessions/session/260209-swift-river-42');
  });
  ...
  it('handles session ID with URL-unsafe characters', () => {
    ...
    expect(buildOAuthDeeplinkUrl(ctx)).toBe('zenskill://allSessions/session/session/with spaces&special=chars');
  });
  ...
  it('constructs correct URL format for instance 2', () => {
    const ctx: OAuthSessionContext = {
      ...
      deeplinkScheme: 'zenskill2',
    };
    expect(buildOAuthDeeplinkUrl(ctx)).toBe('zenskill2://allSessions/session/260111-bold-moon');
  });
});
