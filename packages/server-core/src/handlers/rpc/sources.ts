...
import {
  broadcastZenSkillWriteToolChange,
  registerZenSkillSessionSummarySeam,
} from './zenskill-broadcast'
...
export function registerSourcesHandlers(server: RpcServer, deps: HandlerDeps): void {
  ...
  // Get default permissions from ~/.zenskill/permissions/default.json
  ...
  server.handle('callMcpTool', async (_ctx, workspaceId: string, sourceSlug: string, toolName: string, args: Record<string, unknown>) => {
    ...
    try {
      ...
      // Broadcast change event for ZenSkill write tools (see zenskill-broadcast.ts).
      await broadcastZenSkillWriteToolChange({ server, sessionManager: deps.sessionManager, workspaceId, sourceSlug, toolName, result })
  ...
  // WP-C: session summary → ZenSkill episodes (seam in zenskill-broadcast.ts).
  registerZenSkillSessionSummarySeam({ sessionManager: deps.sessionManager, log })
}
