...
export function registerSystemCoreHandlers(server: RpcServer, deps: HandlerDeps): void {
  ...
  // Shell operations - open URL in external browser (or handle zenskill:// internally)
    ...
    try {
      ...
      if (classification.kind === 'internal-deeplink') {
        ...
        if (deepLink?.navigation?.view || deepLink?.navigation?.action) {
          ...
          deps.platform.logger.info('[OPEN_URL] Routing zenskill:// URL internally via deeplink:navigate')
        ...
        deps.platform.logger.info('[OPEN_URL] Falling back to client openExternal for zenskill:// URL')
