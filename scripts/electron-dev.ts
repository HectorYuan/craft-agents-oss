...
function detectInstance(): void {
  ...
  if (match) {
    ...
    process.env.CRAFT_APP_NAME = `ZenSkill [${instanceNum}]`;
    process.env.CRAFT_CONFIG_DIR = join(process.env.HOME || "", `.craft-agent-${instanceNum}`);
    process.env.CRAFT_DEEPLINK_SCHEME = `zenskill${instanceNum}`;
...
function getElectronEnv(): Record<string, string> {
  ...
  return {
    ...
    CRAFT_APP_NAME: process.env.CRAFT_APP_NAME || "ZenSkill",
    CRAFT_DEEPLINK_SCHEME: process.env.CRAFT_DEEPLINK_SCHEME || "zenskill",
